		...
		foreach ($playlist as $group => $stations) {
			foreach ($stations as $station) {
				if ($station['type'] === 'Live') {
					$tv_show = $station[$name_tag];
					$tv_name = sprintf('%s/%s.strm', $this->clean_filename($group),  $tv_show);
					$this->create_strm_file($this->absolute_filename($directory, $tv_name), $station['url']);
				}
